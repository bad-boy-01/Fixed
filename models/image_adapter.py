...
class LocalImageAdapter:
    ...
    def _save_placeholder(self, output_path: str, w: int = 832, h: int = 480):
        """
        Create a visible placeholder so downstream stages don't crash on a
        missing file. Also writes a .failed sidecar so the renderer can
        detect and skip placeholder images rather than showing them in the
        final video.
        """
        try:
            ...
            # Sidecar marker — renderer checks for this and skips the panel
            with open(output_path + ".failed", "w") as f:
                f.write("generation_failed")
            logger.error(f"  ⚠️  Placeholder saved (generation failed): {os.path.basename(output_path)}")
        except Exception as e:
