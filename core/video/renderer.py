...
class VideoRenderer:
    ...
    def render(self):
        ...
        shot_clips = []
        last_real_img_path = None   # path of the most recent GENERATED image
        current_chapter = -1
        ...
        for panel in panels:
            ...
            if panel.get("merge_with_previous", False):
                # Reuse the last real image (not just current_img_path which
                # might itself be a merged panel — we always want the most
                # recent GENERATED asset, not a recursive reference).
                img_path = last_real_img_path
                if img_path is None:
                    logger.warning(f"Panel {pid} wants to merge but no previous image exists — skipping")
                    continue
            else:
                img_path = os.path.join(self.images_dir, f"{pid}.png")
                if not os.path.exists(img_path):
                    logger.warning(f"Missing image for {pid}: {img_path} — skipping")
                    continue
                # Skip placeholder images (written by _save_placeholder when generation fails)
                if os.path.exists(img_path + ".failed"):
                    logger.warning(f"Image generation failed for {pid} — skipping this panel in video")
                    continue
                last_real_img_path = img_path
