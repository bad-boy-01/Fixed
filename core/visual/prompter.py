    ...
    def generate_prompt_for_panel(self, panel: Dict, chapter_number: int = 1) -> Dict:
        ...
        for char_name in sorted_chars:
            ...
            if cdata:
                ...
                # FIX: Previously did blanket str(v) over every field —
                # list fields (distinctive_features, etc.) were dumped as
                # Python repr: "['innocent expression', 'lying', ...]"
                # and metadata numbers like appearance_confidence (0.9)
                # appeared verbatim in the prompt. Both eat token budget
                # without helping the image model at all.
                NON_VISUAL_KEYS = {"appearance_confidence", "role"}
                traits = []
                for k, v in dna.items():
                    if k in NON_VISUAL_KEYS:
                        continue
                    if isinstance(v, (list, tuple, set)):
                        traits.extend(
                            str(x).strip() for x in v
                            if str(x).strip().lower() not in {"none", "unknown", "not specified", ""}
                        )
                    else:
                        s = str(v).strip()
        ...
        # 4. Assemble — priority order matters for CLIP's 77-token limit:
        # quality → style → framing → world → location → characters → description
        # Style must appear early so it is never the first thing truncated.
        parts = [
            ANIMAGINE_QUALITY,
            MANHWA_STYLE,
            shot_tag,
